...
// }
  ...
  var getFood = function(){
    ...
    if (calories.trim() === "") {
      ...
      var errorCell = document.createElement('p');
      $(".input-calories").append("Please enter a calorie amount")
      errorCell.innerHTML = '<p class="calories-error">Please enter a calorie amount</p>'
      $('input[name="calories"]').after($(errorCell));
    } else if (food.trim() === "") {
      $(".food-error").remove();
      var errorCell = document.createElement('p');
      $('input[name="food"]').after($(errorCell));
    } else {
    $(".calories-error").remove();
