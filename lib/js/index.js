...
function deleteExercise(event, that) {
  event.preventDefault();
...
function submitExercise(food, calories, id){
  var newRow = document.createElement('tr');
  newRow.id = "exercise-"+food+"-"+calories
  var foodCell = document.createElement('td');
  foodCell.innerText = food;
  foodCell.id = "exercise-food-td"
  var calorieCell = document.createElement('td');
  calorieCell.innerText = calories;
  calorieCell.id = "exercise-calories-td"
  var deleteCell = document.createElement('td')
  deleteCell.id = "exercise-delete-"+food+"-"+calories
  deleteCell.innerHTML = '<i id="trash" class="fa fa-trash-o" aria-hidden="true"></i>';
...
function updateExercise() {
  ...
  JSON.parse(localStorage.getItem('exercise')).forEach(function(element, i){
    submitExercise(element.food, element.calories, i)
  })}
}

function storeExercise(food, calories){
  ...
  currentExercise.unshift({food: food, calories: calories});
  exerciseJSON = JSON.stringify(currentExercise);
  localStorage.setItem('exercise', exerciseJSON)

  $("#exerciseTable > tr").remove();
  updateExercise();
}
...
$(document).ready(function(){
  ...
  $('#exerciseTable').on("click", ".fa-trash-o", function(event){
    that = this;
    deleteSnacks(event, that);
  });
